...
def validate_input(selected):
    ...
    try:
        ...
        if len(selected) != 7:
            raise ValueError(
                f"\nExactly 7 values required, you provided {len(selected)}"
            )
...
"""
Once all data is gathered and in al list this will append it to the 
correct Google sheet file using the get_id
"""
...
def main():
    '''
    Runs all code data values, validated, stored then sent to be written to file
    '''
    append_list = []
